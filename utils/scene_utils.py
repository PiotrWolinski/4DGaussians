...
def render_training_image(scene, gaussians, viewpoints, render_func, pipe, background, stage, iteration, time_now, dataset_type):
    def render(gaussians, viewpoint, path, scaling, cam_type):
        ...
        if dataset_type == "PanopticSports":
            ...
            gt_np = rgb_to_grayscale(viewpoint.original_image).permute(1,2,0).cpu().numpy()
        # image_np = rgb_to_grayscale(image, num_output_channels=3).permute(1, 2, 0).cpu().numpy()  # (H, W, 3)
        image_np = rgb_to_grayscale(image).permute(1, 2, 0).cpu().numpy()  # (H, W, 3)
        ...
        clipped = np.clip(image_np,0,1) * 255
        image_with_labels = Image.fromarray(clipped.squeeze(-1).astype('uint8'), mode="L")
